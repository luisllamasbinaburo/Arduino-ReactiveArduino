...
#ifndef _FILTERPASSBAND_h
#define _FILTERPASSBAND_h

template <typename T>
class FilterPassBand : public Operator<T, T>
{
public:
	FilterPassBand<T>(const double alpha1, const double alpha2);
	void OnNext(T value);
...
void FilterPassBand<T>::OnNext(T value)
{
	...
	if (this->_childObserver != nullptr) this->_childObserver->OnNext(_bandPassFilter);
}
