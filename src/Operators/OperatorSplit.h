...
#ifndef _OPERATORSPLIT_h
#define _OPERATORSPLIT_h

template <typename T>
class OperatorSplit : public Operator<T, T>
{
public:
	ReactiveFunction<T> _function;

	OperatorSplit(char separator = ',');
...
template <typename T>
OperatorSplit<T>::OperatorSplit(char separator = ',')
{
	_separator = separator;
}

template <typename T>
void OperatorSplit<T>::OnNext(T value)
{
	size_t counter = 0;
	size_t lastIndex = 0;
	_buffer = value;
