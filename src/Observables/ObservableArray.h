...
#ifndef _OBSERVABLEARRAY_h
#define _OBSERVABLEARRAY_h
...
template <typename T>
inline ObservableArray<T>::ObservableArray(T *array, size_t length)
{
	this->_array = array;
	this->_length = length;
}
...
template<typename T>
inline void ObservableArray<T>::Run()
{
...
template<typename T>
inline void ObservableArray<T>::Reset()
{
	Run();
}
#endif